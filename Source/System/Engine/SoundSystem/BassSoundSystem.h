...
public:
	...
	BassSoundHandle(const UINT handle, const float freq);
	~BassSoundHandle();
...
public:
	...
	BassSoundHandle* LoadSound(std::string filePath, bool bPlay = false);

	//メモリからサウンドのハンドルを作成
	//引数 : void* サウンドが存在するメモリ上の位置, size_t バイト数, bool .flac形式かどうか, bool すぐに再生するか
	//戻り値 : サウンド情報が入っているハンドル
	BassSoundHandle* LoadSound(const void* mem, size_t size, bool bFlacFormat, bool bPlay = false);
...
private:
	//サウンド情報一覧
	std::vector<BassSoundHandle*> m_soundHandles;

	//再生終了時のコールバック関数
	static void CALLBACK PlaybackEndCallback(HSYNC handle, DWORD channel, DWORD data, void* user);

	BassSoundHandle* LoadSoundFromHandle(int handleID, bool bPlay);
};
