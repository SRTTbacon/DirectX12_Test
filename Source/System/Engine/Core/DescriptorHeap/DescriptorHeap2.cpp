...
const unsigned long long DescriptorHeap::STATIC_RESOURCE_COUNT = 1;
...
void DescriptorHeap::SetResource(UINT offset, ID3D12Resource* pResource, DXGI_FORMAT format)
{
    ...
    m_pDevice->CreateShaderResourceView(pResource, &srvDesc, cpuDescriptorHandle);
}
...
D3D12_GPU_DESCRIPTOR_HANDLE DescriptorHeap::GetGpuDescriptorHandle()
{
    D3D12_GPU_DESCRIPTOR_HANDLE gpuDescriptorHandle = m_descriptorHeap->GetGPUDescriptorHandleForHeapStart();
    return gpuDescriptorHandle;
}
