...
{
public:
	VertexBuffer(size_t size, size_t stride, void* pInitData); // コンストラクタでバッファを生成
	D3D12_VERTEX_BUFFER_VIEW View() const; // 頂点バッファビューを取得
	bool IsValid() const; // バッファの生成に成功したかを取得
	void Update(size_t size, size_t stride, void* pInitData);
